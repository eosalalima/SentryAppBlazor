...
public sealed class TurnstileLogPollingWorker : BackgroundService
{
    ...
    {
        while (!stoppingToken.IsCancellationRequested) { try { await controller.WaitUntilActiveAsync(stoppingToken); if (!controller.IsActive) continue; await PollOnceAsync(stoppingToken); await Task.Delay(settings.CurrentValue.PollingInterval, stoppingToken); } catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested) { break; } catch (Exception ex) { logger.LogError(ex, "Turnstile poll failed; it will be retried"); await Task.Delay(settings.CurrentValue.PollingInterval, stoppingToken); } }
    }
